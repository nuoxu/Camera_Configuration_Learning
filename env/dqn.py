...
from env.policy import EpsGreedyQPolicy, GreedyQPolicy
from rl.util import *
import pdb
...
class AbstractDQNAgent(Agent):
    """Write me
    """
    def __init__(self, nb_actions, memory, gamma=.99, batch_size=24, nb_steps_warmup=1000,
                 train_interval=1, memory_interval=1, target_model_update=10000,
                 delta_range=None, delta_clip=np.inf, custom_model_objects={}, env=None, **kwargs):
    ...
    def process_state_batch(self, batch):
        if len(batch) > 1:
            for i in range(len(batch)):
                if len(batch[i][0][0].shape) > 1:
                    batch[i] = np.array(batch[i][0])
                else:
    ...
    def process_state_batch_train(self, batch):
        new_batch = []
        if isinstance(batch[0][0], list):
            for i in range(len(batch[0][0])):
                ...
                for b in batch:
                    temp.append(b[0][i])
                new_batch.append(np.concatenate(temp, 0))
            batch = new_batch
        else:
            batch = np.array(batch)
        if self.processor is None:
    ...
    def compute_batch_q_values(self, state_batch):
        ...
        assert q_values.shape == (1, self.nb_actions)
        return q_values
...
class DQNAgent(AbstractDQNAgent):
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            state0_batch = self.process_state_batch_train(state0_batch)
            state1_batch = self.process_state_batch_train(state1_batch)
