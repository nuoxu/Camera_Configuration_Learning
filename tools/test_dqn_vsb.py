...
if __name__ == '__main__':
    ...
    args = parse_args()
    env = ViewScaleBrightnessSearchEnv(args, is_train=False)
    # env = ViewScaleSearchEnv(args, is_train=False)
    model = get_model(env)
    ...
    search_dict = {}
    for i in range(env.dataset_length):
        state = env.reset()
        done = False

        I0 = env.current_image_name
        B0 = env.benefit
        IN = []
        BN = []
        ...
        while(not done):
            ...
            if not done:
                IN.append(env.current_image_name)
                BN.append(env.benefit)
        ...
        print("START ID: %s, BENEFIT: %f; END ID: %s, BENEFIT: %s." % (I0, B0, IN, BN))
        
        search_dict[I0] = IN

    with open("%s.pickle" % args.pickle_dir, "wb") as fp:
